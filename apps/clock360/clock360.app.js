...
function fixed_from_julian(j_date) {
  const year = j_date[0]
  const month = j_date[1]
  const day = j_date[2]
  const y = year < 0 ? year + 1 : year
  return (
    fixed_from_gregorian([0, 12, 30]) -
    1 +
    365 * (y - 1) +
    quotient(y - 1, 4) +
    quotient(367 * month - 362, 12) +
    (month <= 2 ? 0 : is_julian_leap_year(year) ? -1 : -2) +
    day
  )
}
...
function gregorian_from_fixed(date) {
  const year = gregorian_year_from_fixed(date)
  const prior_days = date - gregorian_new_year(year)
  const correction =
    date < fixed_from_gregorian([year, 3, 1])
      ? 0
      : is_gregorian_leap_year(year)
      ? 1
      : 2
  const month = quotient(12 * (prior_days + correction) + 373, 367)
  const day = 1 + (date - fixed_from_gregorian([year, month, 1]))
...
let newDay = function () {
  ...
  g.setColor('#000000')
    ...
    .setColor('#fafafa')
    .setFont('Vector', 20)
    .drawString('XABCDX'[today.quarter] + today.dayOfQuarter, 0, 0, true)
    .setFont('Vector', 10)
